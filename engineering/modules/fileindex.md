...
- **Views** (all bounded; every response carries a hard cap and, when truncated, says so and how to continue):
...
- `validate_and_index(upload_id, kind, original_filename, path) -> Result<UploadSummary, ValidationError>` — `sessions` passes the user-visible original filename separately from the on-disk path (whose basename it does not promise to preserve); the registry stores it, and rejection messages name it. `UploadSummary` carries the parsed printer name for `.3mf` (binding input) and headline metadata.
- `resolve(session, selector) -> Result<UploadId, QueryError>` — selector is `Newest(kind)` or an explicit identifier.
- Query calls mirroring the views above, each returning a bounded, serializable result for MCP tool responses.
- `remove_session(session)` — drops registry rows and caches for the session's uploads.
...
- `QueryError`: unknown identifier (covers deleted sessions), no upload of the requested kind (the normal no-`.3mf` state of R5), out-of-range window, unsupported view. Structured, surfaced to the agent as a tool-call error, never a panic.
...
7. `Newest(kind)` with no upload of that kind returns the no-upload-of-kind `QueryError`.
...
14. `remove_session` makes every subsequent query for those uploads fail with unknown-identifier.
15. Validation rejections: not-a-zip, zip missing `.3mf` entries, unparseable settings, unrecognizable gcode — each yields its specific `ValidationError` carrying the passed original filename (not the on-disk basename) and leaves no registry row: subsequent queries for the rejected upload answer unknown-identifier. A missing `printer_model` is not among these: it is valid input.
16. A view addressed at the wrong kind (a mesh query on a gcode upload) returns the unsupported-view `QueryError`.
...
19. A crafted `.3mf` without `printer_model` (added to `testdata/`) validates successfully and its summary carries no printer name — absence is valid input, not a rejection.
